...
function getCommodityCost(fuelType, volume, openaiPrice = null) {
  ...
  const marketVariation = 1 + (Math.random() - 0.5) * 0.06;
...
async function calculateDistance(origin, destination, transportMode = 'truck') {
  ...
  if (originCoords && destCoords) {
    ...
    console.log(`📐 Coordinate-based distance: ${distance} miles`);
    
  } else if (openaiService && openaiService.isAvailable) {
    // Use OpenAI for unknown locations
    try {
      console.log(`🤖 Using OpenAI for distance calculation: ${origin} → ${destination} via ${transportMode}`);
      
      const distanceResult = await openaiService.calculateDistanceWithModeAdjustment(
        origin, 
        destination, 
        transportMode
      );
      
      distance = validateNumber(distanceResult.distance_miles, 'OpenAI distance', 1000);
      console.log(`🤖 OpenAI distance result: ${distance} miles`);
      
      // Update city database if we got coordinates
      try {
        const [originData, destData] = await Promise.all([
          openaiService.getLocationCoordinates(origin).catch(() => null),
          openaiService.getLocationCoordinates(destination).catch(() => null)
        ]);
        
        if (originData && validateNumber(originData.lat) && validateNumber(originData.lon)) {
          cityDatabase[origin] = [originData.lat, originData.lon];
          console.log(`📍 Added ${origin} to database: [${originData.lat}, ${originData.lon}]`);
        }
        
        if (destData && validateNumber(destData.lat) && validateNumber(destData.lon)) {
          cityDatabase[destination] = [destData.lat, destData.lon];
          console.log(`📍 Added ${destination} to database: [${destData.lat}, ${destData.lon}]`);
        }
        
      } catch (coordError) {
        console.log('⚠️ Could not get coordinates for future use:', coordError.message);
      }
      
    } catch (aiError) {
      console.log('⚠️ OpenAI distance calculation failed, using deterministic fallback:', aiError.message);
      distance = estimateDistanceFromNames(origin, destination);
    }
    
  } else {
    // Fallback to deterministic calculation
    console.log(`⚠️ No coordinates or AI available, using deterministic approximation`);
    distance = estimateDistanceFromNames(origin, destination);
  }
  
  // Apply transport mode adjustments
  ...
  const multiplier = modeMultipliers[transportMode] || 1.0;
  const adjustedDistance = Math.round(validateNumber(distance, 'base distance', 1000) * multiplier);
  
  // Cache the result
  distanceCache.set(cacheKey, adjustedDistance);
  console.log(`💾 Cached distance for ${transportMode}: ${adjustedDistance} miles`);
  
  return adjustedDistance;
}

// ✅ UPDATED: Multi-modal cost calculation with routing service
async function calculateMultiModalCost(volume, origin, intermediateHub, destination, mode1, mode2, fuelType) {
  let totalCost = 0;
  const validVolume = validateNumber(volume, 'volume', 1);
  
  console.log(`🔄 Calculating multi-modal cost: ${origin} --[${mode1}]--> ${intermediateHub} --[${mode2}]--> ${destination}`);
  
  try {
    // Use routing service for multi-modal route
    ...
    for (const leg of multiModalRoute.legs) {
      try {
        const legCostResult = await calculateBasicCost(validVolume, leg.distance_miles, leg.mode, fuelType);
        const legCost = validateNumber(
          typeof legCostResult === 'object' ? legCostResult.cost : legCostResult,
          `${leg.mode} leg cost`,
          1000
        );
        
        totalCost += legCost;
        console.log(`   Leg ${leg.leg} (${leg.mode}): ${leg.distance_miles} miles = $${legCost.toFixed(2)}`);
        
      } catch (legError) {
        console.log(`⚠️ Error calculating ${leg.mode} leg cost:`, legError.message);
        const fallbackCost = calculateRealisticFallback(validVolume, leg.distance_miles, leg.mode, fuelType);
        totalCost += fallbackCost;
        console.log(`   Fallback cost for ${leg.mode}: $${fallbackCost.toFixed(2)}`);
      }
    }
    
    // Add transfer fees and handling
    const transferFees = 500; // $500 per transfer
    const additionalHandling = validVolume * 25; // $25 per tonne for multi-modal handling
    totalCost += transferFees + additionalHandling;
    ...
    console.log(`   Total time: ${multiModalRoute.total_duration_hours} hours`);
    
    return finalCost;
    
  } catch (error) {
    console.error('❌ Multi-modal routing service failed:', error.message);
    
    // Fallback to simple two-leg calculation
    console.log('🔧 Using fallback multi-modal calculation');
    
    const leg1Distance = await calculateDistance(origin, intermediateHub, mode1, fuelType);
    const leg2Distance = await calculateDistance(intermediateHub, destination, mode2, fuelType);
    
    const leg1Cost = await calculateBasicCost(validVolume, leg1Distance, mode1, fuelType);
    const leg2Cost = await calculateBasicCost(validVolume, leg2Distance, mode2, fuelType);
    
    const totalFallbackCost = (typeof leg1Cost === 'object' ? leg1Cost.cost : leg1Cost) + 
                             (typeof leg2Cost === 'object' ? leg2Cost.cost : leg2Cost) + 
                             500; // Transfer fee
    
    return validateNumber(totalFallbackCost, 'fallback multi-modal cost', 3000);
  }
}
...
function calculateTruckTransitTime(distance, truckCount = 1) {
  ...
  const baseDays = Math.ceil(distance / maxDailyMiles);
  const coordinationDelay = truckCount > 1 ? Math.ceil(truckCount / 3) : 0;
  const totalDays = baseDays + coordinationDelay;
  
  console.log(`🚛 Truck time: ${distance} miles ÷ ${maxDailyMiles} miles/day = ${baseDays} days + ${coordinationDelay} coordination = ${totalDays} days`);
...
async function calculateDistance(origin, destination, transportMode = 'truck') {
  ...
  if (originCoords && destCoords) {
    // Use existing coordinate calculation
    const [lat1, lon1] = originCoords;
    const [lat2, lon2] = destCoords;
    ...
    try {
      ...
      distance = distanceResult.distance_miles;
      console.log(`🤖 OpenAI distance result: ${distance} miles (${distanceResult.route_type})`);
      
      // Also get enhanced location data for future use
      try {
        const [originData, destData] = await Promise.all([
          openaiService.getLocationCoordinatesWithTransport(origin, transportMode, 'hydrogen'),
          openaiService.getLocationCoordinatesWithTransport(destination, transportMode, 'hydrogen')
        ]);
        
        // Update city database with new coordinates if valid
        if (originData.lat && originData.lon) {
          cityDatabase[origin] = [originData.lat, originData.lon];
          console.log(`📍 Added ${origin} to database: [${originData.lat}, ${originData.lon}]`);
        }
  ...
  const adjustedDistance = Math.round(distance * (modeMultipliers[transportMode] || 1.0));
...
async function generateRouteOptions(routeData) {
  const { origin, destination, volume, fuelType, transportMode1 } = routeData;
  const options = [];
  
  console.log('🚦 Generating route options for selected mode:', transportMode1);
  
  // Calculate base distance once
  const baseDistance = await calculateDistance(origin, destination, transportMode1);
  console.log('📐 Base distance calculated:', baseDistance, 'miles');
  
  // Generate options ONLY for the selected transport mode
  const selectedMode = transportMode1;
  
  console.log(`🎯 Generating options for: ${selectedMode}`);
  
  if (selectedMode === 'truck') {
    const truckOptions = await generateTruckOptions(volume, baseDistance, fuelType);
    options.push(...truckOptions);
  } else if (selectedMode === 'ship') {
    const shipOption = await generateShipOption(routeData, baseDistance);
    options.push(shipOption);
  } else if (selectedMode === 'rail') {
    const railOption = await generateRailOption(routeData, baseDistance);
    options.push(railOption);
  } else if (selectedMode === 'pipeline') {
    const pipelineOption = await generatePipelineOption(routeData, baseDistance);
    options.push(pipelineOption);
  }
  
  // Remove duplicates and sort by cost
  const uniqueOptions = options.filter((option, index, self) => 
    index === self.findIndex(o => o.transportModes.join('-') === option.transportModes.join('-'))
  );
  
  console.log(`✅ Generated ${uniqueOptions.length} route options:`, uniqueOptions.map(o => o.name));
  
  return uniqueOptions.sort((a, b) => a.estimatedCost - b.estimatedCost);
}
...
function calculateTruckCost(volume, distance, fuelType, transportFactors, aiEnhanced) {
  ...
  return {
    cost: validateNumber(totalCost, 'truck cost', 1000),
    aiEnhanced,
    aiFactors: transportFactors
  };
}

// ✅ NEW: Rail cost calculation
function calculateRailCost(volume, distance, fuelType, transportFactors, aiEnhanced) {
  let rate = 0.15; // Base rate per tonne-mile
  let fuelMultiplier = getFuelMultiplier(fuelType);
...
function validateNumber(value, fieldName = 'value', defaultValue = 0) {
  ...
  // Ensure positive values for costs and distances
  if (value < 0) {
    console.warn(`⚠️ Negative ${fieldName}: ${value}, using absolute value`);
    return Math.abs(value);
  }
...
// ✅ FIXED: Distance calculation with proper validation
